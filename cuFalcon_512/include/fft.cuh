...
__global__ void recompute_B_fft(fpr *d_f0, fpr *d_f1, fpr *d_f2, fpr *d_f3, fpr *d_f4, fpr *d_f5,
                                const int8_t *G, const int8_t *f, const int8_t *g, const int8_t *F,
...
__device__ __forceinline__ void mq_ntt(uint16_t &reg0, uint16_t &reg1, uint16_t s_ntt[512]) {
    size_t butt_idx;
    uint32_t t;
    uint32_t zeta;
...
// level 1
    ...
    s_ntt[threadIdx.x + 256] = (uint16_t) mq_sub(reg0, t);
    __syncthreads();

// level 2
    butt_idx = (threadIdx.x & 0xFFFFFF80) + threadIdx.x;
    reg1 = s_ntt[butt_idx + 128];
    zeta = GMb[2 + (threadIdx.x >> 7)];
...
__device__  __forceinline__ void mq_intt(uint16_t &reg0, uint16_t &reg1, uint16_t *s_ntt) {
    size_t butt_idx;
    uint32_t t;
...
__device__ __forceinline__ void fft_512(fpr &reg0, fpr &reg1, fpr &reg2, fpr &reg3, fpr *s_f) {


// Level 1
    size_t tmp_idx = (2 + (threadIdx.x >> 7)) << 1;

    fpr zeta1 = fpr_gm_tab[tmp_idx];
    fpr zeta2 = fpr_gm_tab[tmp_idx + 1];
//    __syncthreads();
    FPC_MUL(reg2, reg3, reg2, reg3, zeta1, zeta2);
    FPC_ADD(s_f[threadIdx.x], s_f[threadIdx.x + 256],
            reg0, reg1, reg2, reg3);
    FPC_SUB(s_f[threadIdx.x + 128], s_f[threadIdx.x + 128 + 256],
            reg0, reg1, reg2, reg3);
    __syncthreads();
// Level 2
    size_t butt_idx = (threadIdx.x & 0xFFFFFFC0) + threadIdx.x;
    reg0 = s_f[butt_idx];
    reg1 = s_f[butt_idx + 256];
    reg2 = s_f[butt_idx + 64];
    reg3 = s_f[butt_idx + 64 + 256];

    tmp_idx = (4 + (threadIdx.x >> 6)) << 1;
    __syncthreads();
    zeta1 = fpr_gm_tab[tmp_idx];
    zeta2 = fpr_gm_tab[tmp_idx + 1];

    FPC_MUL(reg2, reg3, reg2, reg3, zeta1, zeta2);
    FPC_ADD(s_f[butt_idx], s_f[butt_idx + 256],
            reg0, reg1, reg2, reg3);
    FPC_SUB(s_f[butt_idx + 64], s_f[butt_idx + 64 + 256],
            reg0, reg1, reg2, reg3);
    __syncthreads();
// Level 3
    butt_idx = (threadIdx.x & 0xFFFFFFE0) + threadIdx.x;
    reg0 = s_f[butt_idx];          // a_re
    reg1 = s_f[butt_idx + 256];    // a_im
    reg2 = s_f[butt_idx + 32];     // b_re
    reg3 = s_f[butt_idx + 32 + 256];  //b_im

    tmp_idx = (8 + (threadIdx.x >> 5)) << 1;
    ...
    FPC_MUL(reg2, reg3, reg2, reg3, zeta1, zeta2);
    FPC_ADD(s_f[butt_idx], s_f[butt_idx + 256 + 128],
            reg0, reg1, reg2, reg3);
    FPC_SUB(s_f[butt_idx + 32 + 16], s_f[butt_idx + 32 + 256 + 16 + 128],
            reg0, reg1, reg2, reg3);
    __syncthreads();
// Level 4
    butt_idx = (threadIdx.x & 0xFFFFFFF0) + threadIdx.x;
    reg0 = s_f[butt_idx + ((butt_idx >> 5) << 4)];
    reg1 = s_f[butt_idx + 256 + 128 + ((butt_idx >> 5) << 4)];
    reg2 = s_f[butt_idx + 16 + ((butt_idx >> 5) << 4)];
    reg3 = s_f[butt_idx + 16 + 256 + 128 + ((butt_idx >> 5) << 4)];

    tmp_idx = (16 + (threadIdx.x >> 4)) << 1;
    ...
    FPC_ADD(s_f[butt_idx], s_f[butt_idx + 256 + 128],
            reg0, reg1, reg2, reg3);
    FPC_SUB(s_f[butt_idx + 16 + 8], s_f[butt_idx + 16 + 8 + 256 + 128],
            reg0, reg1, reg2, reg3);
//    __syncthreads();
// Level 5
    butt_idx = (threadIdx.x & 0xFFFFFFF8) + threadIdx.x;
    reg0 = s_f[butt_idx + ((butt_idx >> 4) << 3)];
    reg1 = s_f[butt_idx + 256 + 128 + ((butt_idx >> 4) << 3)];
    reg2 = s_f[butt_idx + 8 + ((butt_idx >> 4) << 3)];
    reg3 = s_f[butt_idx + 8 + 256 + 128 + ((butt_idx >> 4) << 3)];

    tmp_idx = (32 + (threadIdx.x >> 3)) << 1;
    ...
    FPC_ADD(s_f[butt_idx], s_f[butt_idx + 256 + 128],
            reg0, reg1, reg2, reg3);
    FPC_SUB(s_f[butt_idx + 8 + 4], s_f[butt_idx + 8 + 256 + 128 + 4],
            reg0, reg1, reg2, reg3);
//    __syncthreads();
// Level 6
    butt_idx = (threadIdx.x & 0xFFFFFFFC) + threadIdx.x;
    reg0 = s_f[butt_idx + ((butt_idx >> 3) << 2)];
    reg1 = s_f[butt_idx + 256 + 128 + ((butt_idx >> 3) << 2)];
    reg2 = s_f[butt_idx + 4 + ((butt_idx >> 3) << 2)];
    reg3 = s_f[butt_idx + 4 + 256 + 128 + ((butt_idx >> 3) << 2)];

    tmp_idx = (64 + (threadIdx.x >> 2)) << 1;
    ...
    FPC_ADD(s_f[butt_idx], s_f[butt_idx + 256 + 128],
            reg0, reg1, reg2, reg3);
    FPC_SUB(s_f[butt_idx + 4 + 2], s_f[butt_idx + 4 + 256 + 128 + 2],
            reg0, reg1, reg2, reg3);
//    __syncthreads();
// Level 7
    butt_idx = (threadIdx.x & 0xFFFFFFFE) + threadIdx.x;
    reg0 = s_f[butt_idx + ((butt_idx >> 2) << 1)];
    reg1 = s_f[butt_idx + 256 + 128 + ((butt_idx >> 2) << 1)];
    reg2 = s_f[butt_idx + 2 + ((butt_idx >> 2) << 1)];
    reg3 = s_f[butt_idx + 2 + 256 + 128 + ((butt_idx >> 2) << 1)];

    tmp_idx = (128 + (threadIdx.x >> 1)) << 1;
    ...
    FPC_ADD(s_f[butt_idx], s_f[butt_idx + 256 + 128],
            reg0, reg1, reg2, reg3);
    FPC_SUB(s_f[butt_idx + 2 + 1], s_f[butt_idx + 2 + 256 + 1 + 128],
            reg0, reg1, reg2, reg3);
//    __syncthreads();
// Level 8
    butt_idx = threadIdx.x + threadIdx.x + threadIdx.x;
    reg0 = s_f[butt_idx];
    reg1 = s_f[butt_idx + 256 + 128];
    reg2 = s_f[butt_idx + 1];
    reg3 = s_f[butt_idx + 1 + 256 + 128];

    tmp_idx = (256 + threadIdx.x) << 1;
...
__device__ __forceinline__ void ifft_512(fpr &reg0, fpr &reg1, fpr &reg2, fpr &reg3, fpr *s_f) {
...
// Level 1
    ...
    FPC_MUL(s_f[butt_idx + 1], s_f[butt_idx + 1 + 256 + 128],
            reg0, reg1, zeta1, zeta2);
    __syncthreads();
// Level 2
    butt_idx = (threadIdx.x & 0xFFFFFFFE) + threadIdx.x;
    reg0 = s_f[butt_idx + (butt_idx >> 1)];
    reg1 = s_f[butt_idx + 256 + 128 + (butt_idx >> 1)];
    reg2 = s_f[butt_idx + 2 + (butt_idx >> 1) + 1];
    reg3 = s_f[butt_idx + 2 + 256 + 128 + (butt_idx >> 1) + 1];
    ...
    FPC_ADD(s_f[butt_idx], s_f[butt_idx + 256 + 128],
            reg0, reg1, reg2, reg3);
    FPC_SUB(reg0, reg1, reg0, reg1, reg2, reg3);
    FPC_MUL(s_f[butt_idx + 2], s_f[butt_idx + 2 + 256 + 128],
            reg0, reg1, zeta1, zeta2);
    __syncthreads();
// Level 3
    butt_idx = (threadIdx.x & 0xFFFFFFFC) + threadIdx.x;
    ...
    reg3 = s_f[butt_idx + 4 + 2 + 256 + 128 + ((butt_idx >> 2) << 1)];
    ...
    FPC_ADD(s_f[butt_idx], s_f[butt_idx + 256 + 128],
            reg0, reg1, reg2, reg3);
    FPC_SUB(reg0, reg1, reg0, reg1, reg2, reg3);
    FPC_MUL(s_f[butt_idx + 4], s_f[butt_idx + 4 + 256 + 128],
            reg0, reg1, zeta1, zeta2);
    __syncthreads();
// Level 4
    butt_idx = (threadIdx.x & 0xFFFFFFF8) + threadIdx.x;
    ...
    reg3 = s_f[butt_idx + 8 + 4 + 256 + 128 + ((butt_idx >> 3) << 2)];
    ...
    FPC_ADD(s_f[butt_idx], s_f[butt_idx + 256 + 128],
            reg0, reg1, reg2, reg3);
    FPC_SUB(reg0, reg1, reg0, reg1, reg2, reg3);
    FPC_MUL(s_f[butt_idx + 8], s_f[butt_idx + 8 + 256 + 128],
            reg0, reg1, zeta1, zeta2);
    __syncthreads();
// Level 5
    butt_idx = (threadIdx.x & 0xFFFFFFF0) + threadIdx.x;
    ...
    reg3 = s_f[butt_idx + 16 + 8 + 256 + 128 + ((butt_idx >> 4) << 3)];
    ...
    FPC_ADD(s_f[butt_idx], s_f[butt_idx + 256],
            reg0, reg1, reg2, reg3);
    FPC_SUB(reg0, reg1, reg0, reg1, reg2, reg3);
    FPC_MUL(s_f[butt_idx + 16], s_f[butt_idx + 16 + 256],
            reg0, reg1, zeta1, zeta2);
    __syncthreads();
// Level 6
    butt_idx = (threadIdx.x & 0xFFFFFFE0) + threadIdx.x;
    ...
    reg0 = s_f[butt_idx];
    reg1 = s_f[butt_idx + 256];
    reg2 = s_f[butt_idx + 32];
    reg3 = s_f[butt_idx + 32 + 256];
    ...
    FPC_ADD(s_f[butt_idx], s_f[butt_idx + 256],
            reg0, reg1, reg2, reg3);
    FPC_SUB(reg0, reg1, reg0, reg1, reg2, reg3);
    FPC_MUL(s_f[butt_idx + 32], s_f[butt_idx + 32 + 256],
            reg0, reg1, zeta1, zeta2);
    __syncthreads();
// Level 7
    butt_idx = (threadIdx.x & 0xFFFFFFC0) + threadIdx.x;
    ...
    FPC_ADD(s_f[butt_idx], s_f[butt_idx + 256],
            reg0, reg1, reg2, reg3);
    FPC_SUB(reg0, reg1, reg0, reg1, reg2, reg3);
    FPC_MUL(s_f[butt_idx + 64], s_f[butt_idx + 64 + 256],
            reg0, reg1, zeta1, zeta2);
    __syncthreads();
// Level 8
    butt_idx = threadIdx.x;
    tmp_idx = (2 + (threadIdx.x >> 7)) << 1;
    reg0 = s_f[butt_idx];
    reg1 = s_f[butt_idx + 256];
    reg2 = s_f[butt_idx + 128];
    reg3 = s_f[butt_idx + 128 + 256];
    ...
    FPC_ADD(s_f[butt_idx], s_f[butt_idx + 256],
            reg0, reg1, reg2, reg3);
    FPC_SUB(reg0, reg1, reg0, reg1, reg2, reg3);
    FPC_MUL(s_f[butt_idx + 128], s_f[butt_idx + 128 + 256],
            reg0, reg1, zeta1, zeta2);
}
