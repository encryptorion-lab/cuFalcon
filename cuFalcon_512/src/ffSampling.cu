...
__device__ void poly_merge_fft_s_2(fpr *f, const fpr *f0, const fpr *f1, unsigned logn)
{
    ...
    for (u = threadIdx.x; u < qn; u +=blockDim.x) {
...
__device__  void poly_mul_fft_s_2(
...
__global__ void ffSampling_fft_dyntree(fpr *d_t0, fpr *d_g00, fpr *d_g11, unsigned orig_logn, unsigned logn,
                                       uint64_t *d_scA, uint64_t *d_scdptr, size_t d_mem_pool_pitch)
{
    ...
    stack[0].g00 = d_g00 + blockIdx.x * d_mem_pool_pitch / sizeof(fpr);
    stack[0].g11 = d_g11 + blockIdx.x * d_mem_pool_pitch / sizeof(fpr);
    stack[0].logn = logn;  // 9
    stack[0].is_z0 = 0;
    stack[0].is_z1 = 0;
    ...
    while (1)
    {
        ...
        if (stack[stack_top].logn == 0) {
            ...
            {
                f = stack[stack_top].t0 + 8;
                f0 = stack[stack_top].t0 + 6;
                f1 = stack[stack_top].t0 + 7;
                f[0] = f0[0];
                f[1] = f1[0];
            ...
            {
                f = stack[stack_top].t0;
                f0 = stack[stack_top].t0 + 4;
                f1 = stack[stack_top].t0 + 5;
                f[0] = f0[0];
                f[1] = f1[0];
            }
            __syncwarp(__activemask());
        ...
        else
        {
            ...
            if (!stack[stack_top].is_z1) // 0
            {
                 ...
                 */
                poly_LDL_fft_s_2(stack[stack_top].g00, stack[stack_top].g00 + n, stack[stack_top].g11, stack[stack_top].logn);

                /*
                 * Split d00 and d11 and expand them into half-size quasi-cyclic
                 * Gram matrices. We also save l10 in tmp[].
                 */
                poly_split_fft_s_2(stack[stack_top].t0 + (n << 1), stack[stack_top].t0 + (n << 1) + hn, stack[stack_top].g00, stack[stack_top].logn);
                f = stack[stack_top].g00;
                f0 = stack[stack_top].t0 + (n << 1);
                for(int i=threadIdx.x;i<n;i+=blockDim.x){
                    f[i] = f0[i];
                }
                poly_split_fft_s_2(stack[stack_top].t0 + (n << 1), stack[stack_top].t0 + (n << 1) + hn, stack[stack_top].g11, stack[stack_top].logn);
                f = stack[stack_top].g11;
                f0 = stack[stack_top].t0 + (n << 1);
                for(int i=threadIdx.x;i<n;i+=blockDim.x){
                    f[i] = f0[i];
                }
                f = stack[stack_top].t0 + (n << 1);
                f0 = stack[stack_top].g00 + n;
                for(int i=threadIdx.x;i<n;i+=blockDim.x){
                    f[i] = f0[i];
                }
                f = stack[stack_top].g00 + n;
                f0 = stack[stack_top].g00;
                for(int i=threadIdx.x;i<hn;i+=blockDim.x){
                    f[i] = f0[i];
                }
                f = stack[stack_top].g00 + n + hn;
                f0 = stack[stack_top].g11;
                for(int i=threadIdx.x;i<hn;i+=blockDim.x){
                    f[i] = f0[i];
                }
                /*
                ...
                poly_split_fft_s_2(stack[stack_top].t0 + 3 * n, stack[stack_top].t0 + 3 * n + hn, stack[stack_top].t0 + n, stack[stack_top].logn);
            ...
            else if (!stack[stack_top].is_z0) // 0
            {
                 ...
                 */
                f = stack[stack_top].t0 + 3 * n;
                f0 = stack[stack_top].t0 + n;
                for(int i=threadIdx.x;i<n;i+=blockDim.x){
                    f[i] = f0[i];
                }
                poly_sub_s_2(stack[stack_top].t0 + 3 * n, stack[stack_top].t0 + (n << 2), stack[stack_top].logn);
                f = stack[stack_top].t0 + n;
                f0 = stack[stack_top].t0 + (n << 2);
                for(int i = threadIdx.x; i < n; i+=blockDim.x){
                    f[i] = f0[i];
                }
                poly_mul_fft_s_2(stack[stack_top].t0 + (n << 1), stack[stack_top].t0 + 3 * n, stack[stack_top].logn);
                poly_add_s_2(stack[stack_top].t0, stack[stack_top].t0 + (n << 1), stack[stack_top].logn);
                ...
                poly_split_fft_s_2(stack[stack_top].t0 + (n << 1), stack[stack_top].t0 + (n << 1) + hn, stack[stack_top].t0, stack[stack_top].logn);
            ...
            else
            {
                ...
                else
                {
                    if (!stack[--stack_top].is_z0)
                    {
                        poly_merge_fft_s_2(stack[stack_top].t0 + (n << 3), stack[stack_top].t0 + 6 * n, stack[stack_top].t0 + 7 * n, stack[stack_top].logn);

                    }
                    else
                    {
                        poly_merge_fft_s_2(stack[stack_top].t0, stack[stack_top].t0 + (n << 2), stack[stack_top].t0 + 5 * n, stack[stack_top].logn);
                    }
                }
            }
            __syncwarp(__activemask());
