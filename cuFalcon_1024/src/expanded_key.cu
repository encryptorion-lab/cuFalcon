...
__global__ void sign_tree(fpr *d_t0, fpr *d_t1, fpr *d_b01, fpr *d_b11, const uint16_t *d_hm, int logn,size_t d_mem_pool_pitch)
{
    ...
    reg3 = fpr_of(hm[idx1 + 256 + 512]);

    fft1024(reg0, reg1, reg2, reg3, s_f);

    FPC_ADD(t0[idx2], t0[idx2 + 512],
            reg0, reg1, reg2, reg3);
    FPC_SUB(t0[idx2 + 1], t0[idx2 + 1 + 512],
            reg0, reg1, reg2, reg3);
    __syncthreads();
    ...
    reg0 = t1[threadIdx.x];
    reg1 = t1[threadIdx.x + 512];
    reg2 = b01[threadIdx.x];
    reg3 = b01[threadIdx.x + 512];
    FPC_MUL(t1[threadIdx.x], t1[threadIdx.x + 512], reg0,reg1,reg2,reg3);
    __syncthreads();
    reg0 = t1[threadIdx.x + 256];
    reg1 = t1[threadIdx.x + 512 + 256];
    reg2 = b01[threadIdx.x + 256];
    reg3 = b01[threadIdx.x + 512 + 256];
    FPC_MUL(t1[threadIdx.x + 256], t1[threadIdx.x + 512 + 256], reg0,reg1,reg2,reg3);
    ...
    reg0 = t0[threadIdx.x];
    reg1 = t0[threadIdx.x + 512];
    reg2 = b11[threadIdx.x];
    reg3 = b11[threadIdx.x + 512];
    FPC_MUL(t0[threadIdx.x], t0[threadIdx.x + 512], reg0,reg1,reg2,reg3);

    reg0 = t0[threadIdx.x + 256];
    reg1 = t0[threadIdx.x + 512 + 256];
    reg2 = b11[threadIdx.x + 256];
    reg3 = b11[threadIdx.x + 512 + 256];
    FPC_MUL(t0[threadIdx.x + 256], t0[threadIdx.x + 512 + 256], reg0,reg1,reg2,reg3);
...
__global__ void ffSampling_fft_offline(fpr *d_z0, fpr *d_z1, fpr *d_t0, fpr *d_t1, fpr *d_tree, fpr *d_tmp, unsigned orig_logn, unsigned logn,
                                       uint64_t *d_scA, uint64_t *d_scdptr, size_t d_mem_pool_pitch)
{
    size_t n, hn, i;
    STACK2 stack[LOGN + 1];
    ...
    while (1)
    {
        ...
        if (stack[stack_top].logn == 2) {
            ...
            {
                poly_merge_fft(stack[stack_top].z1, stack[stack_top].tmp, stack[stack_top].tmp + hn,
                               stack[stack_top].logn);
            }
            else
            {
                poly_merge_fft(stack[stack_top].z0, stack[stack_top].tmp, stack[stack_top].tmp + hn,
                               stack[stack_top].logn);
            }
        }
        else
        {
            ...
            if (!stack[stack_top].is_z1) // 0
            {
                ...
                poly_split_fft(stack[stack_top].z1, stack[stack_top].z1 + hn, stack[stack_top].t1,
                               stack[stack_top].logn);
            ...
            else if (!stack[stack_top].is_z0) // 0
            {
                ...
                poly_sub(stack[stack_top].tmp, stack[stack_top].z1, stack[stack_top].logn);
                poly_mul_fft(stack[stack_top].tmp, stack[stack_top].tree, stack[stack_top].logn);
                poly_add(stack[stack_top].tmp, stack[stack_top].t0, stack[stack_top].logn);
                ...
                poly_split_fft(stack[stack_top].z0, stack[stack_top].z0 + hn, stack[stack_top].tmp,
                               stack[stack_top].logn);
                stack[stack_top + 1].z0 = stack[stack_top].tmp;
                stack[stack_top + 1].z1 = stack[stack_top].tmp + hn;
            ...
            else
            {
                ...
                else
                {
                    if (!stack[--stack_top].is_z0)
                    {
                        poly_merge_fft(stack[stack_top].z1, stack[stack_top].tmp, stack[stack_top].tmp + n,
                                       stack[stack_top].logn);
                    }
                    else
                    {
                        poly_merge_fft(stack[stack_top].z0, stack[stack_top].tmp, stack[stack_top].tmp + n,
                                       stack[stack_top].logn);
                    }
                }
            }
...
__global__ void Get_lattice_point(fpr *d_t0, fpr *d_t1, fpr *d_tx, fpr *d_ty, fpr *d_b00, fpr *d_b01, fpr *d_b10, fpr *d_b11, unsigned logn, size_t d_mem_pool_pitch) {
    ...
    a_im = b00[threadIdx.x + 512];
    b_re = tx[threadIdx.x];
    b_im = tx[threadIdx.x + 512];
    FPC_MUL(tmp1, tmp2, a_re, a_im, b_re, b_im);

    a_re = b10[threadIdx.x];
    a_im = b10[threadIdx.x + 512];
    b_re = ty[threadIdx.x];
    b_im = ty[threadIdx.x + 512];
    FPC_MUL(ty[threadIdx.x], ty[threadIdx.x + 512], a_re, a_im, b_re, b_im);

    tx[threadIdx.x] = fpr_add(ty[threadIdx.x], tmp1);
    tx[threadIdx.x + 512] = fpr_add(ty[threadIdx.x + 512], tmp2);

    for (int i = threadIdx.x; i < n; i += blockDim.x) ty[i] = t0[i];

    a_re = b01[threadIdx.x];
    a_im = b01[threadIdx.x + 512];
    b_re = ty[threadIdx.x];
    b_im = ty[threadIdx.x + 512];
    FPC_MUL(ty[threadIdx.x],ty[threadIdx.x + 512], a_re, a_im, b_re, b_im);
    ...
    a_im = b11[threadIdx.x + 512];
    b_re = t1[threadIdx.x];
    b_im = t1[threadIdx.x + 512];
    FPC_MUL(tmp1, tmp2, a_re, a_im, b_re, b_im);

    t1[threadIdx.x] = fpr_add(ty[threadIdx.x], tmp1);
    t1[threadIdx.x + 512] = fpr_add(ty[threadIdx.x + 512], tmp2);
}
