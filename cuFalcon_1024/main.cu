...
int main() {
    ...
    cudaMallocHost((void **) &h_sm, BATCH * stream_num * (MLEN + CRYPTO_BYTES) * sizeof(uint8_t));
    cudaMallocHost((void **) &h_m, BATCH * stream_num * (MLEN + CRYPTO_BYTES) * sizeof(uint8_t));
    ...
    /* ----- sign-1024 ----- */
        ...
        for(int s = 0 ; s < stream_num; s++) {
            for (j = 0; j < BATCH; j++)
                for (i = 0; i < Falcon_N; i++)
                    h_esig[j * (CRYPTO_BYTES - 2 - NONCELEN) + 0 + s * BATCH * (CRYPTO_BYTES - 2 - NONCELEN)] = 0x20 + 10;
            ...
            for(j=0; j<BATCH; j++) for(i=0; i<CRYPTO_SECRETKEYBYTES; i++) h_sk[j*CRYPTO_SECRETKEYBYTES + i + s * BATCH * CRYPTO_SECRETKEYBYTES ] = sk[i];
        }

        uint8_t *d_sign_mem_pool;
        size_t sign_mem_pool_pitch;
        size_t byte_size_per_sign = ALIGN_TO_256_BYTES(CRYPTO_SECRETKEYBYTES) +   // d_sk
                                    ALIGN_TO_256_BYTES(MLEN) +  // d_m
                                    ALIGN_TO_256_BYTES(CRYPTO_BYTES - 2 - NONCELEN) +  // d_esig
                                    ALIGN_TO_256_BYTES(48) +  // d_seed
                                    ALIGN_TO_256_BYTES(NONCELEN) +  // d_nonce
                                    ALIGN_TO_256_BYTES(MLEN + CRYPTO_BYTES) +  // d_sm
                                    ALIGN_TO_256_BYTES(Falcon_N) +  // d_F
                                    ALIGN_TO_256_BYTES(Falcon_N) +  // d_G
                                    ALIGN_TO_256_BYTES(Falcon_N) +  // d_f
                                    ALIGN_TO_256_BYTES(Falcon_N) +  // d_g
                                    ALIGN_TO_256_BYTES(Falcon_N * sizeof(uint16_t)) +  // d_hm
                                    ALIGN_TO_256_BYTES(sizeof(uint32_t)) +  // d_sqn
                                    ALIGN_TO_256_BYTES(sizeof(uint32_t)) +  // d_s
                                    ALIGN_TO_256_BYTES(sizeof(uint32_t)) +  // d_esiglen
                                    ALIGN_TO_256_BYTES(25 * sizeof(uint64_t)) + // d_scA
                                    ALIGN_TO_256_BYTES(sizeof(uint64_t)) +  // d_scdptr
                                    ALIGN_TO_256_BYTES(20 * Falcon_N * sizeof(fpr));  // d_bb


        size_t mem_size_per_sign = byte_size_per_sign; // align to 128 bytes
        cudaMallocPitch(&d_sign_mem_pool, &sign_mem_pool_pitch, mem_size_per_sign, BATCH * stream_num);
        cudaEventRecord(startEvent, 0);
        for(int s = 0 ; s < stream_num; s++) {
            uint8_t *stream_mem_pool = d_sign_mem_pool + s * BATCH * byte_size_per_sign;

            uint8_t *d_sk = stream_mem_pool ;
            ...
            cudaMemcpy2DAsync(d_m, sign_mem_pool_pitch, s_h_m, (MLEN+CRYPTO_BYTES) * sizeof(uint8_t), MLEN * sizeof(uint8_t), BATCH, cudaMemcpyHostToDevice, stream[s]);
            cudaMemcpy2DAsync(d_esig, sign_mem_pool_pitch, s_h_esig, (CRYPTO_BYTES - 2 - NONCELEN) * sizeof(uint8_t), (CRYPTO_BYTES - 2 - NONCELEN) * sizeof(uint8_t), BATCH, cudaMemcpyHostToDevice, stream[s]);
            cudaMemcpy2DAsync(d_seed, sign_mem_pool_pitch, s_h_seed, 48 * sizeof(uint8_t), 48 * sizeof(uint8_t), BATCH, cudaMemcpyHostToDevice, stream[s]);
            cudaMemcpy2DAsync(d_nonce, sign_mem_pool_pitch, s_h_nonce, NONCELEN * sizeof(uint8_t), NONCELEN * sizeof(uint8_t), BATCH, cudaMemcpyHostToDevice, stream[s]);
            cudaMemcpy2DAsync(d_sk, sign_mem_pool_pitch, s_h_sk, CRYPTO_SECRETKEYBYTES * sizeof(uint8_t), CRYPTO_SECRETKEYBYTES * sizeof(uint8_t), BATCH, cudaMemcpyHostToDevice, stream[s]);

            crypto_sign(s_h_sm, s_h_m, stream_mem_pool, sign_mem_pool_pitch,stream[s]);
        ...
        printf("sign_opt-1024:Total execution time for all streams: %.2f ms\n", total_time);
//        printf(" stream_num,BATCH:  %d-%d\n", stream_num,BATCH);
    ...
    size_t byte_size_per_sign = ALIGN_TO_256_BYTES(CRYPTO_SECRETKEYBYTES)
                              + ALIGN_TO_256_BYTES(MLEN)
                              + ALIGN_TO_256_BYTES(CRYPTO_BYTES - 2 - NONCELEN)
                              + ALIGN_TO_256_BYTES(48)  // m
                              + ALIGN_TO_256_BYTES(NONCELEN)
                              + ALIGN_TO_256_BYTES(MLEN+CRYPTO_BYTES)  // d_bb
                              + ALIGN_TO_256_BYTES(Falcon_N * sizeof(uint16_t))
                              + ALIGN_TO_256_BYTES(sizeof(uint32_t))
                              + ALIGN_TO_256_BYTES(sizeof(uint32_t))
                              + ALIGN_TO_256_BYTES(sizeof(uint32_t))
                              + ALIGN_TO_256_BYTES(25 * sizeof(uint64_t))
                              + ALIGN_TO_256_BYTES(sizeof(uint64_t))
                              + ALIGN_TO_256_BYTES(15 * Falcon_N * sizeof(fpr)) // expanded_key
                              + ALIGN_TO_256_BYTES(20 * Falcon_N * sizeof(fpr));


    size_t mem_size_per_sign = byte_size_per_sign; // align to 128 bytes
    cudaMallocPitch(&d_sign_mem_pool, &sign_mem_pool_pitch, mem_size_per_sign, stream_num * BATCH);

    uint8_t *h_seed, *h_nonce, *h_esig, *h_sk;
    fpr *t_h_expanded_key;
    ...
    cudaError_t err1 = cudaMallocHost((void **) &t_h_expanded_key, BATCH*stream_num * 15 * Falcon_N * sizeof(fpr));
    ...
    for(int s=0;s<stream_num;s++){
        ...
        fpr *h_expanded_key  =  t_h_expanded_key   + s * BATCH * 15 * Falcon_N;

        for (j = 0; j < BATCH; j++)
            for (i = 0; i < Falcon_N; i++)
                s_h_esig[j * (CRYPTO_BYTES - 2 - NONCELEN) + 0] = 0x20 + 10;
        ...
        for (j = 0; j < BATCH; j++) for (i = 0; i < CRYPTO_SECRETKEYBYTES; i++) s_h_sk[j * CRYPTO_SECRETKEYBYTES + i] = sk[i];
        for (j = 0; j < BATCH; j++)
            for (i = 0; i < 15 * 1024; i++)
                h_expanded_key[j * (15 * 1024) + i].v = *(double *) &test_expanded_key[i];
    }

    cudaEventRecord(startEvent);
    for(int s = 0;s<stream_num;s++){

        uint8_t *stream_mem_pool_server = d_sign_mem_pool + s * BATCH * byte_size_per_sign;
        ...
        cudaMemcpy2DAsync(d_m, sign_mem_pool_pitch, h_m, (MLEN + CRYPTO_BYTES) * sizeof(uint8_t), MLEN * sizeof(uint8_t),
                          BATCH, cudaMemcpyHostToDevice, stream[s]);
        cudaMemcpy2DAsync(d_esig, sign_mem_pool_pitch, s_h_esig, (CRYPTO_BYTES - 2 - NONCELEN) * sizeof(uint8_t),
                          (CRYPTO_BYTES - 2 - NONCELEN) * sizeof(uint8_t), BATCH, cudaMemcpyHostToDevice, stream[s]);
        cudaMemcpy2DAsync(d_seed, sign_mem_pool_pitch, s_h_seed, 48 * sizeof(uint8_t), 48 * sizeof(uint8_t), BATCH,
                          cudaMemcpyHostToDevice, stream[s]);
        cudaMemcpy2DAsync(d_nonce,sign_mem_pool_pitch, s_h_nonce, NONCELEN * sizeof(uint8_t), NONCELEN * sizeof(uint8_t),
                          BATCH, cudaMemcpyHostToDevice, stream[s]);
        cudaMemcpy2DAsync(d_sk, sign_mem_pool_pitch, s_h_sk, CRYPTO_SECRETKEYBYTES * sizeof(uint8_t),
                          CRYPTO_SECRETKEYBYTES * sizeof(uint8_t), BATCH, cudaMemcpyHostToDevice, stream[s]);
        cudaMemcpy2DAsync(d_expanded_key, sign_mem_pool_pitch, h_expanded_key, 15 * Falcon_N * sizeof(fpr),
                          15 * Falcon_N * sizeof(fpr), BATCH, cudaMemcpyHostToDevice, stream[s]);

        crypto_sign_tree(s_h_sm, s_h_m, stream_mem_pool_server, sign_mem_pool_pitch, stream[s]);
    ...
    cudaEventElapsedTime(&total_time, startEvent, stopEvent);
    printf("sign_tree-1024::Total execution time for all streams: %.2f ms\n", total_time);
//    printf(" stream_num,BATCH:  %d-%d\n", stream_num,BATCH);
    ...
    size_t byte_size_per_sign_server = ALIGN_TO_256_BYTES(CRYPTO_SECRETKEYBYTES) +   // d_sk
                                       ALIGN_TO_256_BYTES(MLEN) +  // d_m
                                       ALIGN_TO_256_BYTES(CRYPTO_BYTES - 2 - NONCELEN) +  // d_esig
                                       ALIGN_TO_256_BYTES(48) +  // d_seed
                                       ALIGN_TO_256_BYTES(NONCELEN) +  // d_nonce
                                       ...
                                       ALIGN_TO_256_BYTES(25 * sizeof(uint64_t)) + // d_scA
                                       ALIGN_TO_256_BYTES(sizeof(uint64_t)) +  // d_scdptr
                                       ALIGN_TO_256_BYTES(10 * Falcon_N * sizeof(fpr));  // d_bb
    ...
    fpr *d_nttbb;
    size_t size = ALIGN_TO_256_BYTES(4 * Falcon_N * sizeof(fpr));
    ...
    for (int s = 0; s < stream_num; s++) {
        ...
        uint8_t *t_h_sm = h_sm + s * BATCH * (MLEN + CRYPTO_BYTES);

        cudaMemcpy2DAsync(d_m, sign_mem_pool_pitch_server, h_m, (MLEN + CRYPTO_BYTES) * sizeof(uint8_t),
        ...
        cudaMemcpy2DAsync(d_esig, sign_mem_pool_pitch_server, s_h_esig, (CRYPTO_BYTES - 2 - NONCELEN) * sizeof(uint8_t),
                          (CRYPTO_BYTES - 2 - NONCELEN) * sizeof(uint8_t), BATCH, cudaMemcpyHostToDevice, stream[s]);
        cudaMemcpy2DAsync(d_seed, sign_mem_pool_pitch_server, s_h_seed, 48 * sizeof(uint8_t), 48 * sizeof(uint8_t), BATCH,
                          cudaMemcpyHostToDevice, stream[s]);
        ...
        crypto_sign_balance(t_h_sm, t_h_m, d_nttbb, stream_mem_pool_server, sign_mem_pool_pitch_server, stream[s]);
    ...
    cudaEventSynchronize(stopEvent);
    cudaEventElapsedTime(&total_time, startEvent, stopEvent);
    printf("sign_balance-1024::Total execution time for all streams: %.2f ms\n", total_time);
//    printf("stream_num,BATCH:   %d-%d\n", stream_num, BATCH);
    cudaDeviceSynchronize();
    ...
    size_t byte_size_per_ver =
            ALIGN_TO_256_BYTES(CRYPTO_PUBLICKEYBYTES) +   // d_pk
            ALIGN_TO_256_BYTES(MLEN + CRYPTO_BYTES) +       // d_sm
            ALIGN_TO_256_BYTES(Falcon_N * sizeof(uint16_t)) +  // d_tmp
            ALIGN_TO_256_BYTES(Falcon_N * sizeof(uint16_t)) +  // d_h
            ALIGN_TO_256_BYTES(Falcon_N * sizeof(uint16_t)) +  // d_sig
            ALIGN_TO_256_BYTES(Falcon_N * sizeof(uint16_t)) +  // d_hm
            ALIGN_TO_256_BYTES(sizeof(uint32_t)) +             // d_s
            ALIGN_TO_256_BYTES(sizeof(uint32_t)) +             // d_smlen
            ALIGN_TO_256_BYTES(sizeof(uint32_t)) +             // d_msg_len
            ALIGN_TO_256_BYTES(sizeof(uint32_t)) +             // d_sig_len
            ALIGN_TO_256_BYTES(25 * sizeof(uint64_t)) +        // d_scA
            ALIGN_TO_256_BYTES(sizeof(uint64_t)) +             // d_scdptr
            ALIGN_TO_256_BYTES(25 * sizeof(uint64_t)) +        // d_test
            ALIGN_TO_256_BYTES(25 * sizeof(uint64_t));         // d_m

    size_t mem_size_per_ver = byte_size_per_ver;
    ...
    for (int s = 0; s < stream_num; s++) {
        ...
        int16_t *d_sig = (int16_t *) (d_hm +
                                      ALIGN_TO_256_BYTES(Falcon_N * sizeof(uint16_t)) / sizeof(uint16_t));  // d_h
        uint32_t *d_s = (uint32_t *) (d_sig +
                                      ALIGN_TO_256_BYTES(Falcon_N * sizeof(uint16_t)) / sizeof(int16_t));  // d_hm
        ...
        uint64_t *d_scA = (uint64_t *) (d_sig_len +
                                        ALIGN_TO_256_BYTES(sizeof(uint32_t)) / sizeof(uint32_t));   // d_sig_len
        ...
        cudaMemcpy2DAsync(d_pk, ver_mem_pool_pitch, s_h_pk, CRYPTO_PUBLICKEYBYTES * sizeof(uint8_t),
                          CRYPTO_PUBLICKEYBYTES * sizeof(uint8_t), BATCH, cudaMemcpyHostToDevice, stream[s]);
        cudaMemcpy2DAsync(d_sm, ver_mem_pool_pitch, s_h_sm, (MLEN + CRYPTO_BYTES) * sizeof(uint8_t),
                          (MLEN + CRYPTO_BYTES) * sizeof(uint8_t), BATCH, cudaMemcpyHostToDevice, stream[s]);
        cudaMemcpy2DAsync(d_test, ver_mem_pool_pitch, s_h_test, 25 * sizeof(uint64_t), 25 * sizeof(uint64_t), BATCH,
                          cudaMemcpyHostToDevice, stream[s]);
        cudaMemcpy2DAsync(d_smlen, ver_mem_pool_pitch, s_h_smlen, sizeof(uint32_t), sizeof(uint32_t), BATCH,
                          cudaMemcpyHostToDevice, stream[s]);
        cudaMemcpy2DAsync(d_msg_len, ver_mem_pool_pitch, s_h_msg_len, sizeof(uint32_t), sizeof(uint32_t), BATCH,
                          cudaMemcpyHostToDevice, stream[s]);
        ...
        cudaMemcpy2DAsync(s_h_tmp, Falcon_N * sizeof(uint16_t), d_tmp, ver_mem_pool_pitch, Falcon_N * sizeof(uint16_t),
                          BATCH, cudaMemcpyDeviceToHost, stream[s]);
    ...
    printf("verify-1024: Total execution time for all streams: %.2f ms\n", total_time_ver);
//    printf("   %d-%d\n", stream_num, BATCH);
