 ...
 */
__constant__ static const uint16_t GMb[] = {
 ...
 */
__constant__ static const uint16_t iGMb[] = {
...
__global__ void recompute_B_fft(fpr *d_f0, fpr *d_f1, fpr *d_f2, fpr *d_f3, fpr *d_f4, fpr *d_f5, const int8_t *G, const int8_t *f, const int8_t *g, const int8_t *F, size_t d_mem_pool_pitch);

__global__ void sign_balance(fpr *d_b0, const fpr *d_nntbb, uint16_t *d_hm, size_t d_mem_pool_pitch);

__global__ void arrange_kernel(fpr *d_f0, fpr *d_f1, fpr *d_f2, fpr *d_f3, fpr *d_f4, fpr *d_f5, fpr *d_ntt, size_t d_mem_pool_pitch);
...
#define FPC_MUL(d_re, d_im, a_re, a_im, b_re, b_im)   do { \
        ...
        tmp = fpr_mul(fpct_a_im, fpct_b_re);\
        fpct_d_im = fpr_mac(fpct_a_re, fpct_b_im, tmp); \
        (d_re) = fpct_d_re; \
        (d_im) = fpct_d_im; \
    } while (0)
...
__device__ static inline uint32_t mq_montymul_2(uint32_t x, uint32_t y)
{
    uint32_t z, w;
...
__device__ __forceinline__ void mq_ntt1024(uint16_t &reg0,uint16_t &reg1, uint16_t s_ntt[1024]){
...
// level 0
    t = mq_montymul(reg1, 7888);
    s_ntt[threadIdx.x] = (uint16_t)mq_add(reg0,  t);
    s_ntt[threadIdx.x + 512] = (uint16_t)mq_sub(reg0,  t);
    __syncthreads();

// level 1
    butt_idx = (threadIdx.x & 0xFFFFFF00) + threadIdx.x;
    reg1 = s_ntt[butt_idx + 256];
    zeta = GMb[2 + (threadIdx.x >> 8)];
    t = mq_montymul(reg1, zeta);
    reg0 = s_ntt[butt_idx];
    s_ntt[butt_idx] = (uint16_t)mq_add(reg0,  t);
    s_ntt[butt_idx + 256] = (uint16_t)mq_sub(reg0,  t);
    __syncthreads();

// level 2
    butt_idx = (threadIdx.x & 0xFFFFFF80) + threadIdx.x;
    reg1 = s_ntt[butt_idx + 128];
    zeta = GMb[4 + (threadIdx.x >> 7)];
    t = mq_montymul(reg1, zeta);
    reg0 = s_ntt[butt_idx];
    s_ntt[butt_idx] = (uint16_t)mq_add(reg0,  t);
    s_ntt[butt_idx + 128] = (uint16_t)mq_sub(reg0,  t);
    __syncthreads();

// level 3
    butt_idx = (threadIdx.x & 0xFFFFFFC0) + threadIdx.x;
    reg1 = s_ntt[butt_idx + 64];
    zeta = GMb[8 + (threadIdx.x >> 6)];
    t = mq_montymul(reg1, zeta);
    reg0 = s_ntt[butt_idx];
    s_ntt[butt_idx] = (uint16_t)mq_add(reg0,  t);
    s_ntt[butt_idx + 64] = (uint16_t)mq_sub(reg0,  t);
    __syncthreads();

// level 4
    butt_idx = (threadIdx.x & 0xFFFFFFE0) + threadIdx.x;
    reg1 = s_ntt[butt_idx + 32];
    zeta = GMb[16 + (threadIdx.x >> 5)];
    t = mq_montymul(reg1, zeta);
    reg0 = s_ntt[butt_idx];
    s_ntt[butt_idx] = (uint16_t)mq_add(reg0,  t);
    s_ntt[butt_idx + 32] = (uint16_t)mq_sub(reg0,  t);
    __syncthreads();

// level 5
    butt_idx = (threadIdx.x & 0xFFFFFFF0) + threadIdx.x;
    reg1 = s_ntt[butt_idx + 16];
    zeta = GMb[32 + (threadIdx.x >> 4)];
    t = mq_montymul(reg1, zeta);
    reg0 = s_ntt[butt_idx];
    s_ntt[butt_idx] = (uint16_t)mq_add(reg0,  t);
    s_ntt[butt_idx + 16] = (uint16_t)mq_sub(reg0,  t);
    __syncthreads();

// level 6
    butt_idx = (threadIdx.x & 0xFFFFFFF8) + threadIdx.x;
    reg1 = s_ntt[butt_idx + 8];
    zeta = GMb[64 + (threadIdx.x >> 3)];
    t = mq_montymul(reg1, zeta);
    reg0 = s_ntt[butt_idx];
    s_ntt[butt_idx] = (uint16_t)mq_add(reg0,  t);
    s_ntt[butt_idx + 8] = (uint16_t)mq_sub(reg0,  t);
    __syncthreads();

// level 7
    butt_idx = (threadIdx.x & 0xFFFFFFFC) + threadIdx.x;
    reg1 = s_ntt[butt_idx + 4];
    zeta = GMb[128 + (threadIdx.x >> 2)];
    t = mq_montymul(reg1, zeta);
    reg0 = s_ntt[butt_idx];
    s_ntt[butt_idx] = (uint16_t)mq_add(reg0,  t);
    s_ntt[butt_idx + 4] = (uint16_t)mq_sub(reg0,  t);
    __syncthreads();

// level 8
    butt_idx = (threadIdx.x & 0xFFFFFFFE) + threadIdx.x;
    reg1 = s_ntt[butt_idx + 2];
    zeta = GMb[256 + (threadIdx.x >> 1)];
    t = mq_montymul(reg1, zeta);
    reg0 = s_ntt[butt_idx];
    s_ntt[butt_idx] = (uint16_t)mq_add(reg0,  t);
    s_ntt[butt_idx + 2] = (uint16_t)mq_sub(reg0,  t);
    __syncthreads();

// level 9
    butt_idx = threadIdx.x  + threadIdx.x;
    reg1 = s_ntt[butt_idx + 1];
    zeta = GMb[512 + threadIdx.x];
    t = mq_montymul(reg1, zeta);
    reg0 = s_ntt[butt_idx];
    reg1 = (uint16_t)mq_sub(reg0,  t);
    reg0 = (uint16_t)mq_add(reg0,  t);
    __syncthreads();

}

__device__  __forceinline__ void mq_intt1024(uint16_t &reg0,uint16_t &reg1, uint16_t *s_ntt){
    size_t butt_idx;
    uint32_t t;

// level 9
    butt_idx = 2 * threadIdx.x;
    t = reg0;
    s_ntt[butt_idx] = (uint16_t)mq_add(t, reg1);
    reg1 = mq_sub(t, reg1);
    t = iGMb[512 + threadIdx.x];
    s_ntt[butt_idx + 1] = (uint16_t)mq_montymul(reg1, t);
    __syncthreads();
...
// level 8
    ...
    s_ntt[butt_idx] = (uint16_t)mq_add(t, reg1);
    reg1 = mq_sub(t, reg1);
    t = iGMb[256 + (threadIdx.x >> 1)];
    s_ntt[butt_idx + 2] = (uint16_t)mq_montymul(reg1, t);
    __syncthreads();
...
// level 7
    ...
    s_ntt[butt_idx] = (uint16_t)mq_add(t, reg1);
    reg1 = mq_sub(t, reg1);
    t = iGMb[128 + (threadIdx.x >> 2)];
    s_ntt[butt_idx + 4] = (uint16_t)mq_montymul(reg1, t);
    __syncthreads();
...
// level 6
    ...
    s_ntt[butt_idx] = (uint16_t)mq_add(t, reg1);
    reg1 = mq_sub(t, reg1);
    t = iGMb[64 + (threadIdx.x >> 3)];
    s_ntt[butt_idx + 8] = (uint16_t)mq_montymul(reg1, t);
    __syncthreads();
...
// level 5
    ...
    s_ntt[butt_idx] = (uint16_t)mq_add(t, reg1);
    reg1 = mq_sub(t, reg1);
    t = iGMb[32 + (threadIdx.x >> 4)];
    s_ntt[butt_idx + 16] = (uint16_t)mq_montymul(reg1, t);
    __syncthreads();
...
// level 4
    ...
    s_ntt[butt_idx] = (uint16_t)mq_add(t, reg1);
    reg1 = mq_sub(t, reg1);
    t = iGMb[16 + (threadIdx.x >> 5)];
    s_ntt[butt_idx + 32] = (uint16_t)mq_montymul(reg1, t);
    __syncthreads();
    ...
    s_ntt[butt_idx] = (uint16_t)mq_add(t, reg1);
    reg1 = mq_sub(t, reg1);
    t = iGMb[8 + (threadIdx.x >> 6)];
    s_ntt[butt_idx + 64] = (uint16_t)mq_montymul(reg1, t);
    __syncthreads();
...
// level 2
    ...
    s_ntt[butt_idx] = (uint16_t)mq_add(t, reg1);
    reg1 = mq_sub(t, reg1);
    t = iGMb[4 + (threadIdx.x >> 7)];
    s_ntt[butt_idx + 128] = (uint16_t)mq_montymul(reg1, t);
    __syncthreads();
...
// level 1
    ...
    s_ntt[butt_idx] = (uint16_t)mq_add(t, reg1);
    reg1 = mq_sub(t, reg1);
    t = iGMb[2 + (threadIdx.x >> 8)];
    s_ntt[butt_idx + 256] = (uint16_t)mq_montymul(reg1, t);
    __syncthreads();

// level 0
    reg0 = s_ntt[threadIdx.x];
    reg1 = s_ntt[threadIdx.x + 512];
    t = reg0;
    reg0 = (uint16_t)mq_add(t, reg1);
    reg1 = mq_sub(t, reg1);
    reg1 = (uint16_t)mq_montymul(reg1, 4401);
    __syncthreads();

}

__device__ __forceinline__ void fft1024(fpr &reg0, fpr &reg1, fpr &reg2, fpr &reg3, fpr *s_f){

    size_t tmp_idx = (2 + (threadIdx.x >> 8)) << 1;

    fpr zeta1 = fpr_gm_tab[tmp_idx];
    fpr zeta2 = fpr_gm_tab[tmp_idx + 1];

    FPC_MUL(reg2, reg3, reg2, reg3, zeta1, zeta2);
    FPC_ADD(s_f[threadIdx.x], s_f[threadIdx.x + 512],
            reg0, reg1, reg2, reg3);
    FPC_SUB(s_f[threadIdx.x + 256], s_f[threadIdx.x + 256 + 512],
            reg0, reg1, reg2, reg3);
    __syncthreads();
    // Level 1
    size_t butt_idx = (threadIdx.x & 0xFFFFFF80) + threadIdx.x;
    reg0 = s_f[butt_idx];
    reg1 = s_f[butt_idx + 512];
    reg2 = s_f[butt_idx + 128];
    reg3 = s_f[butt_idx + 128 + 512];

    tmp_idx = (4 + (threadIdx.x >> 7)) << 1;

    zeta1 = fpr_gm_tab[tmp_idx];
    zeta2 = fpr_gm_tab[tmp_idx + 1];
    __syncthreads();
    FPC_MUL(reg2, reg3, reg2, reg3, zeta1, zeta2);
    FPC_ADD(s_f[butt_idx], s_f[butt_idx + 512],
            reg0, reg1, reg2, reg3);
    FPC_SUB(s_f[butt_idx + 128], s_f[butt_idx + 128 + 512],
            reg0, reg1, reg2, reg3);
    __syncthreads();
// Level 2
    butt_idx = (threadIdx.x & 0xFFFFFFC0) + threadIdx.x;
    reg0 = s_f[butt_idx];
    reg1 = s_f[butt_idx + 512];
    reg2 = s_f[butt_idx + 64];
    reg3 = s_f[butt_idx + 64 + 512];

    tmp_idx = (8 + (threadIdx.x >> 6)) << 1;
    ...
    FPC_ADD(s_f[butt_idx], s_f[butt_idx + 512],
            reg0, reg1, reg2, reg3);
    FPC_SUB(s_f[butt_idx + 64], s_f[butt_idx + 64 + 512],
            reg0, reg1, reg2, reg3);
    __syncthreads();
// Level 3
    butt_idx = (threadIdx.x & 0xFFFFFFE0) + threadIdx.x;
    reg0 = s_f[butt_idx];          // a_re
    reg1 = s_f[butt_idx + 512];    // a_im
    reg2 = s_f[butt_idx + 32];     // b_re
    reg3 = s_f[butt_idx + 32 + 512];  //b_im

    tmp_idx = (16 + (threadIdx.x >> 5)) << 1;
    ...
    FPC_ADD(s_f[butt_idx], s_f[butt_idx + 512], //
            reg0, reg1, reg2, reg3);
    FPC_SUB(s_f[butt_idx + 32], s_f[butt_idx + 32 + 512], //
            reg0, reg1, reg2, reg3);
    __syncthreads();
...
// Level 4
    ...
    reg0 = s_f[butt_idx];
    reg1 = s_f[butt_idx + 512];
    reg2 = s_f[butt_idx + 16];
    reg3 = s_f[butt_idx + 16 + 512];

    tmp_idx = (32 + (threadIdx.x >> 4)) << 1;

    zeta1 = fpr_gm_tab[tmp_idx];
    zeta2 = fpr_gm_tab[tmp_idx + 1];
    __syncthreads();
    butt_idx += ((butt_idx >> 4) << 3);
    FPC_MUL(reg2, reg3, reg2, reg3, zeta1, zeta2);
    FPC_ADD(s_f[butt_idx], s_f[butt_idx + 512 + 256],
            reg0, reg1, reg2, reg3);
    FPC_SUB(s_f[butt_idx + 16 + 8], s_f[butt_idx + 16 + 8 + 512 + 256],
            reg0, reg1, reg2, reg3);
    __syncthreads();
// Level 5
    butt_idx = (threadIdx.x & 0xFFFFFFF8) + threadIdx.x;
    reg0 = s_f[butt_idx + ((butt_idx >> 4) << 3)];
    reg1 = s_f[butt_idx + 512 + 256 + ((butt_idx >> 4) << 3)];
    reg2 = s_f[butt_idx + 8 + ((butt_idx >> 4) << 3)];
    reg3 = s_f[butt_idx + 8 + 512 + 256 + ((butt_idx >> 4) << 3)];

    tmp_idx = (64 + (threadIdx.x >> 3)) << 1;
    zeta1 = fpr_gm_tab[tmp_idx];
    zeta2 = fpr_gm_tab[tmp_idx + 1];
    __syncthreads();
    butt_idx += ((butt_idx >> 3) << 2);
    FPC_MUL(reg2, reg3, reg2, reg3, zeta1, zeta2);
    FPC_ADD(s_f[butt_idx], s_f[butt_idx + 512 + 256],
            reg0, reg1, reg2, reg3);
    FPC_SUB(s_f[butt_idx + 8 + 4], s_f[butt_idx + 8 + 512 + 256 + 4],
            reg0, reg1, reg2, reg3);
    __syncthreads();
// Level 6
    butt_idx = (threadIdx.x & 0xFFFFFFFC) + threadIdx.x;
    reg0 = s_f[butt_idx + ((butt_idx >> 3) << 2)];
    reg1 = s_f[butt_idx + 512 + 256 + ((butt_idx >> 3) << 2)];
    reg2 = s_f[butt_idx + 4 + ((butt_idx >> 3) << 2)];
    reg3 = s_f[butt_idx + 4 + 512 + 256 + ((butt_idx >> 3) << 2)];

    tmp_idx = (128 + (threadIdx.x >> 2)) << 1;

    zeta1 = fpr_gm_tab[tmp_idx];
    zeta2 = fpr_gm_tab[tmp_idx + 1];
    __syncthreads();
    butt_idx += ((butt_idx >> 2) << 1);
    FPC_MUL(reg2, reg3, reg2, reg3, zeta1, zeta2);
    FPC_ADD(s_f[butt_idx], s_f[butt_idx + 512 + 256],
            reg0, reg1, reg2, reg3);
    FPC_SUB(s_f[butt_idx + 4 + 2], s_f[butt_idx + 4 + 512 + 256 + 2],
            reg0, reg1, reg2, reg3);
    __syncthreads();
// Level 7
    butt_idx = (threadIdx.x & 0xFFFFFFFE) + threadIdx.x;
    reg0 = s_f[butt_idx + ((butt_idx >> 2) << 1)];
    reg1 = s_f[butt_idx + 512 + 256 + ((butt_idx >> 2) << 1)];
    reg2 = s_f[butt_idx + 2 + ((butt_idx >> 2) << 1)];
    reg3 = s_f[butt_idx + 2 + 512 + 256 + ((butt_idx >> 2) << 1)];

    tmp_idx = (256 + (threadIdx.x >> 1)) << 1;
    ...
    FPC_ADD(s_f[butt_idx], s_f[butt_idx + 512 + 256],
            reg0, reg1, reg2, reg3);
    FPC_SUB(s_f[butt_idx + 2 + 1], s_f[butt_idx + 2 + 512 + 1 + 256],
            reg0, reg1, reg2, reg3);
    __syncthreads();
// Level 8
    butt_idx = threadIdx.x + threadIdx.x + threadIdx.x;
    reg0 = s_f[butt_idx];
    reg1 = s_f[butt_idx + 512 + 256];
    reg2 = s_f[butt_idx + 1];
    reg3 = s_f[butt_idx + 1 + 256 + 512];

    tmp_idx = (512 + threadIdx.x) << 1;
    __syncthreads();
    zeta1 = fpr_gm_tab[tmp_idx];
    zeta2 = fpr_gm_tab[tmp_idx + 1];
...
__device__ __forceinline__ void ifft1024(fpr &reg0, fpr &reg1, fpr &reg2, fpr &reg3, fpr *s_f){

    // Level 8
    size_t butt_idx = threadIdx.x + threadIdx.x;
    size_t tmp_idx = (512 + threadIdx.x) << 1;
    fpr zeta1 = fpr_gm_tab[tmp_idx];
    fpr zeta2 = fpr_neg(fpr_gm_tab[tmp_idx + 1]);
    __syncthreads();
    butt_idx += (butt_idx >> 1);
    FPC_ADD(s_f[butt_idx], s_f[butt_idx + 256 + 512],
            reg0, reg1, reg2, reg3);
    FPC_SUB(reg0, reg1, reg0, reg1, reg2, reg3);
    FPC_MUL(s_f[butt_idx + 1], s_f[butt_idx + 1 + 256 + 512],
            reg0, reg1, zeta1, zeta2);
    __syncthreads();

    // Level 7
    butt_idx = (threadIdx.x & 0xFFFFFFFE) + threadIdx.x;
    reg0 = s_f[butt_idx + (butt_idx >> 1)];
    reg1 = s_f[butt_idx + 512 + 256 + (butt_idx >> 1)];
    reg2 = s_f[butt_idx + 2 + (butt_idx >> 1) + 1];
    reg3 = s_f[butt_idx + 2 + 512 + 256 + (butt_idx >> 1) + 1];
    tmp_idx = (256 + (threadIdx.x >> 1)) << 1;
    zeta1 = fpr_gm_tab[tmp_idx];
    zeta2 = fpr_neg(fpr_gm_tab[tmp_idx + 1]);
    __syncthreads();
    butt_idx += ((butt_idx >> 2) << 1);
    FPC_ADD(s_f[butt_idx], s_f[butt_idx + 256 + 512],
            reg0, reg1, reg2, reg3);
    FPC_SUB(reg0, reg1, reg0, reg1, reg2, reg3);
    FPC_MUL(s_f[butt_idx + 2], s_f[butt_idx + 2 + 256 + 512],
            reg0, reg1, zeta1, zeta2);
    __syncthreads();
    // Level 6
    butt_idx = (threadIdx.x & 0xFFFFFFFC) + threadIdx.x;
    reg0 = s_f[butt_idx + ((butt_idx >> 2) << 1)];
    reg1 = s_f[butt_idx + 512 + 256 + ((butt_idx >> 2) << 1)];
    reg2 = s_f[butt_idx + 4 + 2 + ((butt_idx >> 2) << 1)];
    reg3 = s_f[butt_idx + 4 + 2 + 512 + 256 + ((butt_idx >> 2) << 1)];
    tmp_idx = (128 + (threadIdx.x >> 2)) << 1;
    zeta1 = fpr_gm_tab[tmp_idx];
    zeta2 = fpr_neg(fpr_gm_tab[tmp_idx + 1]);
    __syncthreads();
    ...
    FPC_ADD(s_f[butt_idx], s_f[butt_idx + 256 + 512],
            reg0, reg1, reg2, reg3);
    FPC_SUB(reg0, reg1, reg0, reg1, reg2, reg3);
    FPC_MUL(s_f[butt_idx + 4], s_f[butt_idx + 4 + 256 + 512],
            reg0, reg1, zeta1, zeta2);
    __syncthreads();
    // Level 5
    butt_idx = (threadIdx.x & 0xFFFFFFF8) + threadIdx.x;
    reg0 = s_f[butt_idx + ((butt_idx >> 3) << 2)];
    reg1 = s_f[butt_idx + 512 + 256 + ((butt_idx >> 3) << 2)];
    reg2 = s_f[butt_idx + 8 + 4 + ((butt_idx >> 3) << 2)];
    reg3 = s_f[butt_idx + 8 + 4 + 512 + 256 + ((butt_idx >> 3) << 2)];
    tmp_idx = (64 + (threadIdx.x >> 3)) << 1;
    zeta1 = fpr_gm_tab[tmp_idx];
    zeta2 = fpr_neg(fpr_gm_tab[tmp_idx + 1]);
    __syncthreads();
    ...
    FPC_ADD(s_f[butt_idx], s_f[butt_idx + 256 + 512],
            reg0, reg1, reg2, reg3);
    FPC_SUB(reg0, reg1, reg0, reg1, reg2, reg3);
    FPC_MUL(s_f[butt_idx + 8], s_f[butt_idx + 8 + 256 + 512],
            reg0, reg1, zeta1, zeta2);
    __syncthreads();
    ...
    reg0 = s_f[butt_idx + ((butt_idx >> 4) << 3)];
    reg1 = s_f[butt_idx + 512 + 256 + ((butt_idx >> 4) << 3)];
    reg2 = s_f[butt_idx + 16 + 8 + ((butt_idx >> 4) << 3)];
    reg3 = s_f[butt_idx + 16 + 8 + 512 + 256 + ((butt_idx >> 4) << 3)];
    tmp_idx = (32 + (threadIdx.x >> 4)) << 1;
    zeta1 = fpr_gm_tab[tmp_idx];
    zeta2 = fpr_neg(fpr_gm_tab[tmp_idx + 1]);
    __syncthreads();
    FPC_ADD(s_f[butt_idx], s_f[butt_idx + 512],
            reg0, reg1, reg2, reg3);
    FPC_SUB(reg0, reg1, reg0, reg1, reg2, reg3);
    FPC_MUL(s_f[butt_idx + 16], s_f[butt_idx + 16 + 512],
            reg0, reg1, zeta1, zeta2);
    __syncthreads();
    ...
    FPC_ADD(s_f[butt_idx], s_f[butt_idx + 512],
            reg0, reg1, reg2, reg3);
    FPC_SUB(reg0, reg1, reg0, reg1, reg2, reg3);
    FPC_MUL(s_f[butt_idx + 32], s_f[butt_idx + 32 + 512],
            reg0, reg1, zeta1, zeta2);
    __syncthreads();
    ...
    FPC_ADD(s_f[butt_idx], s_f[butt_idx + 512],
            reg0, reg1, reg2, reg3);
    FPC_SUB(reg0, reg1, reg0, reg1, reg2, reg3);
    FPC_MUL(s_f[butt_idx + 64], s_f[butt_idx + 64 + 512],
            reg0, reg1, zeta1, zeta2);
    __syncthreads();
    ...
    FPC_ADD(s_f[butt_idx], s_f[butt_idx + 512],
            reg0, reg1, reg2, reg3);
    FPC_SUB(reg0, reg1, reg0, reg1, reg2, reg3);
    FPC_MUL(s_f[butt_idx + 128], s_f[butt_idx + 128 + 512],
            reg0, reg1, zeta1, zeta2);
    __syncthreads();
    ...
    reg2 = s_f[idx1 + 256];
    reg1 = s_f[idx1 + 512];
    reg3 = s_f[idx1 + 256 + 512];
    tmp_idx = (2 + (threadIdx.x >> 8)) << 1;
    zeta1 = fpr_gm_tab[tmp_idx];
    zeta2 = fpr_neg(fpr_gm_tab[tmp_idx + 1]);
    __syncthreads();
    FPC_ADD(s_f[threadIdx.x], s_f[threadIdx.x + 512],
            reg0, reg1, reg2, reg3);
    FPC_SUB(reg0, reg1, reg0, reg1, reg2, reg3);
    FPC_MUL(s_f[threadIdx.x + 256], s_f[threadIdx.x + 256 + 512],
            reg0, reg1, zeta1, zeta2);
